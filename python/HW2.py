# Reade Hauge 
# Sources include class notes, Rephactor text book and for the last problem I went to the TA for help. 
...
def exchange(str):
    ...
    """
    if len(str) > 1:
        last_letter = str[-1:] #accessing last letter in list using indexing 
        first_letter = str[0:1] #accessing the first letter in list using indexing
        return(last_letter + str[1:-1] + first_letter)
    else: 
        return(str) #if there is only one letter, return that one letter 
...
large_squares = [ val for val in squares if val >= 10000] #optional if statement to limit the val size 
print(large_squares)
    """
    new_range = [value for value in alist if value > max or value < min] #set min and max values as constraints
    return(new_range)
...
def topping1(dictionary): #for this exercise I am going off of our class lecture on dictionaries
    """
    ...
    """
    if "ice cream" in dictionary: #if key is in dictionary 
        dictionary["ice cream"] = "cherry" #then make sure this is the value for that key 
    dictionary["bread"]= "butter" #always have this key / value combo 
    return(dictionary)
...
def word_count(strings):
    ...
    *** Important: your code must use comprehensions and should not be more than two
    ...
    a: count (value = 2)
    #Here we are using {} which indicates a dictionary 
    Letter is the key

    """
    wordcount = {letter: strings.count(letter) for letter in strings} #key: count the number of letters for letter in the given string
    return(wordcount)
...
def friend_list(friend_dictionary): #for this problem I went to the TA office hours for help 
    """
    ...
    Keep going through and append values to that key 
    """
    new_dictionary = {}
    for key, value in friend_dictionary.items(): 
        if key not in new_dictionary:
            new_dictionary[key] = [value] #this adds the new key to the list 
        else: 
            new_dictionary[key].append(value) #if key is in dictionary append the value to key to make sure we have all keys
        
        if value not in new_dictionary: 
            new_dictionary[value] = [key]
        else: 
            new_dictionary[value].append(key) #always use [] when accessing dictionary item 
